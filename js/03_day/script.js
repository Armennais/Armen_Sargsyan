...
var arr = [
    { CategoryId: "aaa", CategoryName: "Category 1", CategoryColor: "cgreen" }
    , { CategoryId: "a", CategoryName: "Category 2", CategoryColor: "purple" }
    , { CategoryId: "a", CategoryName: "Category 1", CategoryColor: "cgreen" }
  ]
  
  var tempResult = {}
  
  arr.forEach((n) => 
    tempResult[n.CategoryId] =  tempResult[n.CategoryId] ? tempResult[n.CategoryId] + 1 : 1
  )
...
const newlist = data => data.map((n) => ({
	name: n.name,
	position: n.hr.position,
	salary: n.hr.salary
}));
...
console.log(filterBySalary(obj));
// 3.Գտնել ամենից վաղ աշխատակցին; ( new Date() )
const earliestEmployee = (data) => data.sort(
	(a, b) => new Date(b.hr.start_date).getTime() - new Date(a.hr.start_date).getTime())[data.length - 1];
console.log(earliestEmployee(obj));
// 4.Ստանալ այն աշխատակիցների ցուցակը, որոնց աշխատավարձը <= 100000 և ավելացնել նոր դաշտ՝ բոնուս, որի
//    արժեքը = աշխատավարձի 20%;
let setBonusField = (n) => ({...n,
	hr: {...n.hr,
			bonus: `${Number(n.hr.salary.replace(/\D/g, '') * 20) / 100}`
	}
});
const newlistWithBonus = data => data.filter((n) => Number(n.hr.salary.replace(/\D/g, '')) <= 100000).map(setBonusField);
console.log(newlistWithBonus(obj));
...
//       ...
//    }
const cities = obj.map((n) => n.contact[0]);
...
const mainFunc = (data) => {
	...
	setTimeout(() => {
		let employeesName = data.map((n) => n.name);
		console.log(`Մեր բոլոր աշխատակիցները։ ${employeesName} `);
		const getRandEmployee = employeesName[Math.floor(Math.random() * employeesName.length)];
		console.log(`Ներող ապեր քեզ դուրս ենք հանում: ${getRandEmployee}`);
		data = data.filter((n) => n.name !== getRandEmployee);
		let newEmployeesName = data.map((n) => n.name);
		console.log(`Նոր աշխատակիցների ցուցակ։ ${newEmployeesName}`);
	}, 3000)
};
mainFunc(obj);
